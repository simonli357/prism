...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for img_path in images_list:
            ...
            print("Processing image: ", os.path.join(args.r, img_name))
            img = cv2.imread(os.path.join(args.r, img_name),
                               cv2.IMREAD_COLOR)
            if img is None:
                print("Image not found: ", os.path.join(args.r, img_name))
                exit()
            # resize if not 2048x1024
            resized = False
            old_shape = img.shape
            ...
            print("Save image: ", sv_path+img_name)
            if resized:
