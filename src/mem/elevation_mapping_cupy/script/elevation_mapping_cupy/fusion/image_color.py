...
def color_correspondences_to_map_kernel(resolution, width, height):
    """
    Creates a CuPy ElementwiseKernel to project RGB colors from an image onto a semantic map.
    This corrected version uses a robust if/else structure to explicitly ignore black pixels.
    """
    color_correspondences_to_map_kernel = cp.ElementwiseKernel(
        ...
        operation=string.Template(
            ...
            // Check for a valid correspondence first.
            if (valid_correspondence[cell_idx]) {
                int cell_idx_2 = get_map_idx(i, 1);

                // Calculate the pixel index in the flattened RGB image array.
                ...
                // Now, check if the valid pixel is NOT black.
                if (r > 0 || g > 0 || b > 0) {
                    // CONDITIONS MET: Update the map with the new, non-black color.
                    unsigned int rgb = (r << 16) + (g << 8) + b;
                    float rgb_ = __uint_as_float(rgb);
                    new_sem_map[get_map_idx(i, map_idx)] = rgb_;
                } else {
                    // PIXEL IS BLACK: Ignore it and keep the old map value.
                    new_sem_map[get_map_idx(i, map_idx)] = sem_map[get_map_idx(i, map_idx)];
                }
            } else {
                // NO VALID CORRESPONDENCE: Keep the old map value.
                new_sem_map[get_map_idx(i, map_idx)] = sem_map[get_map_idx(i, map_idx)];
            }
            """
        ).substitute(),
