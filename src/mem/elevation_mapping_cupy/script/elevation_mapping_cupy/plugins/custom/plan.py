...
def yaw_from_quaternion(x, y, z, w):
    """Return yaw (rotation about z) from a quaternion (world->body orientation)."""
...
def world_to_body(dx_w, dy_w, yaw):
    """
    Transform a vector from world frame to body (ego) frame.
    Body frame: +x forward, +y left.
    R_body_to_world = [[c,-s],[s,c]], so world->body uses R^T = [[c,s],[-s,c]].
    """
    c = math.cos(yaw)
    s = math.sin(yaw)
    dx_b =  c * dx_w + s * dy_w
...
class BiAStarPlanner:
    """
    Bidirectional A* over a cost grid.
    - Works on non-uniform cost maps (finite = traversable; inf = obstacle).
    - Crops to a minimal ROI around start/goal (+ margin) to shrink memory/expansions.
    - Uses closed sets to avoid re-expansions.
    - Stops on time/expansion budget and returns best partial path if possible.
    - Optional heuristic weight (>1.0) to speed up with near-optimal paths.
    """
    ...
    def plan(self, cost_map, start, goal,
             ...
             roi_margin_cells=60):
        """
        cost_map: 2D np.ndarray, finite => traversable cost (>=0), inf => obstacle
        start, goal: (r, c) in *full* map indices
        Returns list of (r, c) in full map indices, or None.
        """
        ...
        # ROI crop to shrink problem
        r0,r1,c0,c1 = self._crop_roi(start, goal, rows, cols, roi_margin_cells)
        sub = cost_map[r0:r1+1, c0:c1+1]
        H, W = sub.shape

        # Remap start/goal into ROI coords
        s = (start[0]-r0, start[1]-c0)
        g = (goal[0]-r0,  goal[1]-c0)

        # Quick sanity: if ROI is tiny or blocked around goal, we still try but budgets will cut it off.

        # Data structures (forward + backward)
        inf = np.inf
        ...
        # Helper to relax neighbors from one frontier
        def expand(openQ, g_this, f_this, g_other, closed_this, came_this,
            ...
            cr, cc = current
            # Early exit if meet the other closed set (frontiers touched)
            if closedB[cr, cc] if forward else closedF[cr, cc]:
                # Potential full path via current
            ...
            for dr, dc in self.neigh:
                ...
                if tentative_g < g_this[nr, nc]:
                    ...
                    # f = g + w*h (towards the *true* target)
                    ...
                    # If this node is already reached by the other search, update meet
        ...
        while (openF or openB):
            ...
            # Always expand the side with smaller current f-top to be goal-directed
            ...
            # If we have a meet point and best f-values exceed best_cost, we can stop
            if best_meet is not None:
                # Lower bounds from both queues
                lb = inf
        ...
        if best_meet is None:
            # Try direct hit (one frontier reached the other goal)
            if gF[g] < inf:
                path_roi = self._reconstruct(cameF, g, s, g, None)
            elif gB[s] < inf:
                path_roi = self._reconstruct(cameB, s, g, s, None)
            else:
                # As a fallback, pick the best seen node from forward side
                # to at least move toward goal.
                # (Pick min f among discovered nodes.)
        ...
        # Map ROI coords back to full map indices
        path_full = [(r0 + r, c0 + c) for (r, c) in path_roi]
        return path_full
      
class AStarPlanner:
    """A* with closed set + optional time/node budget and weighted heuristic."""
    ...
    def plan(self, cost_map, start, goal, timeout_sec=0.5, max_expansions=80000):
        ...
        # For stable heap ordering
        push_idx = 1

        # Track best-so-far in case we stop early
        best_node = start
        ...
        while open_set:
            ...
            if (time.monotonic() - t0) > timeout_sec or expansions >= max_expansions:
                ...
                # If we have progressed toward the goal, return partial path
                if best_node != start:
            ...
            # Skip if a better path to current was already processed
            if closed[current]:
                continue
            closed[current] = True
            expansions += 1
...
class RobotCentricPlanner:
    ...
    def odom_callback(self, msg: Odometry):
        ...
        # Use the odom header frame as our "world" frame for publishing
        self.world_frame = msg.header.frame_id

    def map_callback(self, msg: GridMapMsg):
        """
        Process the incoming robot-centric grid map.
        Assumes [0,0] is top-left, robot at the center.
        """
        # Prefer the message header for the frame id
        ...
        # GridMap: dim[0] = COLS (x), dim[1] = ROWS (y)
        cols = layout.dim[0].size
        rows = layout.dim[1].size

        data_1d = np.array(layer_data_msg.data, dtype=np.float32)
        # Column-major in message → reshape (cols, rows) with Fortran order
        trav_map_col_major = data_1d.reshape((cols, rows), order='F')
        # Transpose to standard NumPy (rows, cols), [0,0] top-left
        trav_map_numpy = trav_map_col_major.T

        # Convert traversability ∈ [0,1] → cost; unknown/NaN → inf (obstacle)
    ...
    def goal_callback(self, msg: PoseStamped):
        ...
        # 1) world -> body (ego) transform of the goal vector
        dx_w = gx - rx
        dy_w = gy - ry
        goal_x_body, goal_y_body = world_to_body(dx_w, dy_w, yaw)

        # goal_x_body is +forward, goal_y_body is +left
        ...
        # 2) Convert ego (meters) -> grid cell offsets (NumPy indexing)
        #    rows increase downward, cols increase to the right
        #    so +forward (ego +x) means negative row delta; +left (ego +y) means negative col delta
        ...
        # 3) Clamp to reachable radius (inside the grid), if requested
        ...
        # 5) Bounds check (after clamping it should be ok)
        ...
        # 7) Build Path in world frame (manual ego->world transform)
        path_msg = Path()
